...
/** @param {string} text */
function parseWslVersionOutput(text) {
  const version = {};
  const wslMatch = text.match(/WSL[^:\n]*:\s*(\d+\.\d+\.\d+(?:\.\d+)?)/i)
    || text.match(/Версия\s+WSL\s*:\s*(\d+\.\d+\.\d+(?:\.\d+)?)/iu);
  if (wslMatch) version.wsl = wslMatch[1];
  const kernelMatch = text.match(/Kernel[^:\n]*:\s*(\S+)/i)
    || text.match(/ядра\s*:\s*(\S+)/iu);
  if (kernelMatch) version.kernel = kernelMatch[1];
  return version;
}

function readWslVersion() {
  try {
    const buf = execSync('wsl --version', {
      windowsHide: true,
      env: wslEnv,
      stdio: ['pipe', 'pipe', 'pipe'],
    });
    const text = typeof buf === 'string' ? buf : decodeText(buf);
    const parsed = parseWslVersionOutput(text);
    return parsed.wsl ? parsed : null;
  } catch {
    return null;
  }
}

/** @param {string} output JSON from `wslc ps -a --format json` */
function parseWslcContainerListJson(output) {
  let list;
  try {
  ...
  for (const item of list) {
    const status = String(
      item?.Status ?? item?.status ?? item?.State?.Status ?? item?.state?.status ?? '',
    ).toLowerCase();
    if (/running|^up\b/.test(status)) running++;
    else stopped++;
  }
  return { running, stopped, total: running + stopped };
}

function isWslcCliAvailable() {
  try {
    execFileSync('wslc', ['--version'], {
      encoding: 'utf8',
      timeout: 5000,
      windowsHide: true,
      stdio: ['pipe', 'pipe', 'pipe'],
...
/**
 * Host-level WSL info (version + WSL Containers stats).
 * @returns {{ version: Object|null, wslc: { running: number, stopped: number, total: number }|null }}
 */
function getWslHostInfo() {
  const version = readWslVersion();
  if (!isWslcCliAvailable()) {
    return { version, wslc: null };
  }

  try {
    const output = execFileSync('wslc', ['ps', '-a', '--format', 'json'], {
      encoding: 'utf8',
      timeout: 10000,
      windowsHide: true,
      stdio: ['pipe', 'pipe', 'pipe'],
    }).toString();
    const stats = parseWslcContainerListJson(output);
    return { version, wslc: stats ?? { running: 0, stopped: 0, total: 0 } };
  } catch {
    // wslc is installed but listing failed — still show the card with zeros.
    return { version, wslc: { running: 0, stopped: 0, total: 0 } };
  }
}

/**
 * List WSL 2 distributions.
 * @returns {{ ok: boolean, distros?: Array, defaultDistro?: string, version?: Object, error?: string }}
 */
function checkWsl() {
  try {
    const output = execSync('wsl -l -v', {
      encoding: 'utf16le',
      stdio: ['pipe', 'pipe', 'pipe'],
      windowsHide: true,
      env: wslEnv,
    ...
    return { ok: true, distros, defaultDistro, version: readWslVersion() };
  } catch (err) {
...
/**
 * Detect Windows-host tools (not available inside a distro).
 * @returns {Object<string, boolean>}
 */
function detectHostTools() {
  return { wslc: isWslcCliAvailable() };
}

const ALLOWED_WSLC_PRUNE_TARGETS = new Set(['image', 'container', 'volume', 'network']);

function isAllowedWslcPruneStep(parts) {
  if (parts.length < 2 || parts.length > 3) return false;
  if (!ALLOWED_WSLC_PRUNE_TARGETS.has(parts[0])) return false;
  if (parts[1] !== 'prune') return false;
  if (parts.length === 2) return true;
  // wslc uses -f/--filter (requires a value), not docker-style force; allow --all only.
  return (parts[2] === '-a' || parts[2] === '--all')
    && (parts[0] === 'image' || parts[0] === 'volume');
}

/**
 * Run an allowlisted wslc maintenance command on the Windows host.
 * Command uses `|` to separate steps, e.g. `image prune|container prune|volume prune`.
 * @param {Object} opts
 ...
 */
function runHostCleanupTask({ taskId, command, onOutput }) {
  const steps = String(command || '').split('|').map(s => s.trim()).filter(Boolean);
  if (steps.length === 0) {
    return Promise.resolve({ ok: false, output: 'Empty host command.', code: -1 });
  }

  const runStep = (args) => new Promise((resolve) => {
    const proc = spawn('wslc', args, { windowsHide: true });
    let output = '';
    ...
    proc.on('close', (code) => resolve({ ok: code === 0, output, code }));
    proc.on('error', (err) => resolve({ ok: false, output: friendlyError(err.message), code: -1 }));
  });

  return (async () => {
    for (const step of steps) {
      const parts = step.split(/\s+/);
      if (!isAllowedWslcPruneStep(parts)) {
        return { ok: false, output: 'Disallowed wslc host command step.\n', code: -1 };
      }
    }
    ...
    try {
      ...
      const msg = 'wslc is not installed — skipping WSL Containers cleanup\n';
      if (onOutput) onOutput({ taskId, text: msg });
      return { ok: true, output: msg, code: 0 };
    }

    let fullOutput = '';
    for (const step of steps) {
      const parts = step.split(/\s+/);
      const result = await runStep(parts);
      fullOutput += result.output;
      if (!result.ok) return { ...result, output: fullOutput };
    }
    return { ok: true, output: fullOutput, code: 0 };
  })();
}
